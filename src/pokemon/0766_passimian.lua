  ...
  receive_card = function(self, card, to_key, context)
    if to_key and G.P_CENTERS[to_key].stage then
      ...
      elseif context and context.card and context.card.ability then
        -- Making a list of values *not* to pass along
        local exceptions = {}
        ...
        -- Save all the values from the received card (minus exceptions)
        for k, v in pairs(context.card.ability) do
          if not PkmnDip.utils.contains(exceptions, v) then
            values_to_keep[k] = type(v) == 'table' and copy_table(v) or v
          end
        end
      end
      ...
      if not edition then
        ...
        sound = G.P_CENTERS[edition.key].sound
        sound.pitch = edition.type == 'poke_shiny' and 1 or 2
        local ARGS = {message = localize('poke_receiver_ex'), colour = G.C.DARK_EDITION}; for k, v in pairs(sound) do ARGS[k] = v end
        card:juice_up(1, 0.5)
        card_eval_status_text(card, 'extra', nil, nil, nil, ARGS)
